...
// Generic paperwork every new supplier needs, per the same Vietnamese
// food-safety compliance requirements documented for Kamereo below (Kitchen
// Food Safety Book Section 3.3) — attached automatically to every supplier
// added through the app so onboarding a new one never skips the compliance
// step. Items only apply where relevant (e.g. the liquor licence only if
// alcohol is ordered from them).
export const STANDARD_SUPPLIER_DOC_CHECKLIST: SupplierDocItem[] = [
  { label: { en: "Copy of business registration certificate on file", vi: "Bản sao giấy ĐKKD lưu hồ sơ" }, checked: false },
  { label: { en: "Food safety / HACCP certificate for their facility", vi: "Giấy chứng nhận ATTP/HACCP của cơ sở" }, checked: false },
  { label: { en: "Wholesale liquor licence copy — only if ordering alcohol through them", vi: "Bản sao giấy phép bán buôn rượu — chỉ khi đặt rượu qua nhà cung cấp này" }, checked: false },
  { label: { en: "VAT invoice for every order — request and file each one", vi: "Hóa đơn VAT cho mỗi đơn — yêu cầu và lưu từng hóa đơn" }, checked: false },
  { label: { en: "Delivery slip for every order — file with the invoice", vi: "Phiếu giao hàng cho mỗi đơn — lưu cùng hóa đơn" }, checked: false },
